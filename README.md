...
npm run build:firefox   # Firefox (desktop) build -> dist-firefox/
npm run lint:firefox    # web-ext lint dist-firefox/ (AMO pre-check)
```

The same source also builds a **desktop Firefox** package for
[AMO](https://addons.mozilla.org/), sharing the codebase through a `browser.*`
shim; `npm run build` and the Chrome output are unchanged. Firefox for Android is
out of scope for now. See [`PORTING-FIREFOX.md`](./PORTING-FIREFOX.md).

CI (`.github/workflows/ci.yml`) runs the same steps (Chrome **and** Firefox) on
every push and pull request, and uploads the built `dist/` as an artifact. On
`main` it then packs and publishes a release — see below.
